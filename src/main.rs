...
fn main() {
    ...
    system.main_loop(move |_, ui, window| {
        ...
        let mut strings = get_strings(&app.engine.as_ref().unwrap()).unwrap();
        ...
        app.read_position = (
            closest_str.position.corners()[0].x - 50.,
            closest_str.position.corners()[0].y,
        );
        app.to_read = closest_str.str.clone();
        println!(
            "Closest string is '{:?}' at '{:?}'",
            app.to_read, app.read_position
        );

        println!("Creating window at {:?}", app.read_position);
        window.set_outer_position(winit::dpi::PhysicalPosition {
            x: app.read_position.0,
            y: app.read_position.1,
        });
