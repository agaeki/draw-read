    ...
    pub fn draw_rect(&mut self) {
        if let Some(rect_start) = self.rect_start {
            ...
            for i in img_rect_start.0..img_rect_end.0 {
                // for j_min
                let left_index = (((width * img_rect_start.1) + i) * 4) as usize;
                // for j_max
                let right_index = (((width * img_rect_end.1) + i) * 4) as usize;
            ...
            for j in img_rect_end.1..img_rect_start.1 {
                // for i_min
                let top_index = (((width * j) + img_rect_start.0) * 4) as usize;
                // for i_max
                let bottom_index = (((width * j) + img_rect_end.0) * 4) as usize;
